<template>
  <div class="flex flex-col gap-6">
    <HistogramSlider class="range-slider" :style="rangeSliderStyle" :width="300" :barWidth="8"
      :data="histogramRangeSliderStore.properties[props.propName].arr" :drag-interval="false" :force-edges="false"
      :grid="false" :hideFromTo="true" :primaryColor="primaryColor" :holderColor="holderColor"
      :handleSize="mediumHandleSize" :barRadius="mediumRoundRadius"
      :min="histogramRangeSliderStore.properties[props.propName].min"
      :max="histogramRangeSliderStore.properties[props.propName].max" :from="selectedRange.from" :to="selectedRange.to"
      :step="props.step" :clip="false" :histSliderGap="6" :lineHeight="24" @finish="handleRangeSliderSelectFinish"
      @change="handleRangeSliderSelectChange" />
    <div class="flex">
      <div class="flex flex-col mr-auto">
        <p>Min</p>
        <select @change="handleOptionSelect" v-model="selectedMinRange" class="select select-sm  font-semibold text-lg">
          <option v-for="[index, value] of minRangeArr.entries()" :key="index" :value="value">{{ value }}</option>
        </select>
      </div>
      <div class="flex flex-col ml-auto">
        <p>Max</p>
        <select @change="handleOptionSelect" v-model="selectedMaxRange" class="select select-sm  font-semibold text-lg">
