<template>
  <main>
    <div class="hidden md:flex md:flex-col gap-4">
      <div role="tablist" class="tabs tabs-border mx-auto tabs-lg w-full">
        <input type="radio" name="my_tabs_1" role="tab" class="tab tabs-border" aria-label="Chemical"
          checked="checked" />
        <div role="tabpanel" class="border-t-2 border-t-gray-100/30 tab-content p-10">
          <div class="join w-full">
            <div class="relative w-full">
              <input class="input  join-item text-2xl w-full py-8 pr-10" v-model="querySearchByRepr"
                @keydown.enter="handleSearchByRepresentation"
                data-typewriter="LSOA5108276818...;N-(2-Aminoethyl)-D-gluconamide...;C1=CC=C(C=C1)C=O...;[X3&H0]...;C25H20O4...;InChI=1S/C3H6O/c1-3(2)4/h1-2H3..."
                type="text" required>
              <button type="submit"
                class="absolute inset-y-0 right-0 flex items-center px-2 btn btn-ghost bg-100 m-auto mr-2 rounded-lg"
                @click="clearInput">
            ...
            <div class="indicator">
              <button type="submit" class="btn btn-primary btn-outline h-full join-item"
                @click="handleSearchByRepresentation">
        ...
        <input type="radio" name="my_tabs_1" role="tab" class="tab tabs-border" aria-label="Citation" />
        <div role="tabpanel" class="border-t-2 border-t-gray-100/30 tab-content p-10">
          <div class="join w-full">
            <div class="relative w-full">
              <input class="input  join-item text-2xl w-full py-8 pr-10" v-model="querySearchByCitation"
                @keydown.enter="handleSearchByCitation"
                data-typewriter="Search DOI or Title...;10.1016/j.bioorg.2020.103984..." type="text" required>
              <button type="submit"
                class="absolute inset-y-0 right-0 flex items-center px-2 btn btn-ghost m-auto mr-2 rounded-lg"
                @click="clearInput">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                  stroke="currentColor" class="size-10">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" />
    ...
    <div class="flex flex-col md:hidden gap-2">
      <select v-model="searchFieldSelected" class="select  text-lg w-full">
      ...
      <div class="join w-full" v-if="searchFieldSelected === searchFieldOptions[0]['value']">
        <div class="relative w-full">
          <input class="input  join-item text-lg w-full pr-10" type="text" placeholder="Search Chemical" required
            v-model="querySearchByRepr" @keydown.enter="handleSearchByRepresentation">
          <button type="submit" class="absolute inset-y-0 right-0 flex items-center px-2 m-auto btn btn-ghost"
            @click="clearInput">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
        ...
        <div class="indicator">
          <button type="submit" class="btn btn-primary btn-outline join-item" @click="handleSearchByRepresentation">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
      ...
      <div class="join w-full" v-if="searchFieldSelected === searchFieldOptions[1]['value']">
        <div class="relative w-full">
          <input class="input  join-item text-lg w-full pr-10" type="text" v-model="querySearchByCitation"
            @keydown.enter="handleSearchByCitation" placeholder="Search DOI or Title" required>
          <button type="submit" class="absolute inset-y-0 right-0 flex items-center px-2 m-auto btn btn-ghost"
            @click="clearInput">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="size-6">
        ...
        <div class="indicator">
          <button type="submit" class="btn btn-primary btn-outline join-item" @click="handleSearchByCitation">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5"
              stroke="currentColor" class="h-7 w-7">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
