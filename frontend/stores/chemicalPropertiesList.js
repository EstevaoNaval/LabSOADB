...
export const useChemicalPropertiesListStore = defineStore('chemicalPropertiesList', {
    ...
    actions: {
        async fetchAllChemicalProperties() {
            ...
            this.properties = {
                count_lipinski_violation: response.data.results.count_lipinski_violation.map(utils.roundValue) || [],
                count_pains_alert: response.data.results.count_pains_alert.map(utils.roundValue) || [],
                jplogp: response.data.results.jplogp.map(utils.roundValue) || [],
                molecular_weight: response.data.results.molecular_weight.map(utils.roundValue) || [],
                tpsa: response.data.results.tpsa.map(utils.roundValue) || [],
                mp_lower_bound: response.data.results.mp_lower_bound.map(utils.roundValue) || [],
                mp_upper_bound: response.data.results.mp_upper_bound.map(utils.roundValue) || [],
                h_bond_acceptor: response.data.results.h_bond_acceptor.map(utils.roundValue) || [],
                h_bond_donor: response.data.results.h_bond_donor.map(utils.roundValue) || [],
