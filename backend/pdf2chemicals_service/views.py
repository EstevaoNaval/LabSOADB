...
@extend_schema(
    ...
    responses={
        202: OpenApiResponse(
            description="Files successfully enqueued for processing."
        ),
        400: OpenApiResponse(
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            for file in uploaded_files:
                ...
                random_name = generate_random_alphanumeric_sequence(FILE_RANDOM_NAME_SIZE)
                pdf_file_path = default_storage.save(
                    f"tmp_pdfs/{random_name}.pdf",
                    file
                )
            ...
            logger.error(
                f"Error processing PDF upload from user {user_id}: {e}",
                exc_info=True
            )
            ...
            if created_task_ids:
                try:
                    ...
                    logger.warning(
                        f"Failed to cleanup UserTask records: {db_error}"
                    )
