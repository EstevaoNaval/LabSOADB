...
def create_pbs_script_task(self, *args, **kwargs):
    ...
    ✅ Passes parent_task_id to next task
...
def load_chemical_from_json(self, *args, **kwargs):
    ...
    if self.check_revocation(parent_task_id):
        logger.warning(f"Revoked before JSON loading")
        return {'revoked': True, 'parent_task_id': parent_task_id}
...
def post_chemicals_in_db(self, result_data, user_id, parent_task_id):
    """
    ...
    Args:
        result_data: Dict with chemical_list and parent_task_id
        user_id: User ID for database association
        parent_task_id: Parent task ID for revocation check
...
def return_pdf2chemicals_task_final_result(self, chemical_count_data, **kwargs):
    """
    ...
    Args:
        chemical_count_data: Dict with chemical_count from post_chemicals_in_db
