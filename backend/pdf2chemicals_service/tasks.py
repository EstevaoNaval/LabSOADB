...
def extract_and_save_chemicals_from_pdf(
    ...
    try:
        ...
        logger.error(f"[ORCHESTRATION] Failed to submit workflow: {e}", exc_info=True)
...
def handle_pdf2chemicals_task_error(self, *args, **kwargs):
    ...
    logger.error(f"[ERROR_HANDLER] Workflow failed: {parent_task_id}")
    ...
    if self.check_revocation(parent_task_id):
        logger.info(f"[ERROR_HANDLER] Task was revoked, cleaning up {parent_task_id}")

        _queue_cleanup(parent_task_id, reason="revocation", cleanup_data=cleanup_data)
    ...
    retry_count, max_exceeded = TaskRetryTracker.increment_and_check(
        parent_task_id, max_retries=5
    )

    logger.info(f"[ERROR_HANDLER] Retry attempt {retry_count}/5 for {parent_task_id}")
    ...
    if max_exceeded:
        ...
        # Queue cleanup
        _queue_cleanup(
            parent_task_id, reason="failure_max_retries", cleanup_data=cleanup_data
        )
    ...
    try:
        ...
        logger.error(f"[ERROR_HANDLER] Failed to schedule retry: {e}", exc_info=True)
        ...
        # Queue cleanup
        _queue_cleanup(
            parent_task_id, reason="failure_retry_scheduling", cleanup_data=cleanup_data
        )
...
def _queue_cleanup(parent_task_id, reason="unknown", cleanup_data=None):
    ...
    try:
        ...
        logger.info(f"[CLEANUP] Queued cleanup task for {parent_task_id} ({reason})")
    except Exception as e:
        logger.error(f"[CLEANUP] Failed to queue cleanup: {e}", exc_info=True)
...
def create_pbs_script_task(self, *args, **kwargs):
    ...
    try:
        ...
        if not cluster_node_manager.is_node_reservation_valid(
...
def monitor_pdf2chemicals_job(self, *args, **kwargs):
    ...
    try:
        ...
        if self.check_revocation(parent_task_id):
            ...
            return {**kwargs, "status": "revoked", "stage": "monitor", "revoked": True}
...
def load_chemical_from_json(self, *args, **kwargs):
    ...
    logger.info(f"[LOAD_JSON] Loading chemicals from job {job_id} on {node_name}")
    ...
    try:
        ...
        logger.debug(
            f"[LOAD_JSON] Job {job_id} completed. Checking for JSON at {json_filepath}"
        )
...
def post_chemicals_in_db(self, *args, **kwargs):
    ...
    logger.info(f"[POST_DB] Posting {len(chemical_list)} chemicals for user {user_id}")
    ...
    try:
        ...
        post_chemical_tasks = group(
            post_chemical.s(chemical_data, user_id) for chemical_data in chemical_list
        )
        ...
        return {**kwargs, "status": "success", "stage": "post_db"}
...
def return_pdf2chemicals_task_final_result(self, *args, **kwargs):
    ...
    try:
        ...
        output_filepath = os.path.join(output_dir, f"{output_filename}.{export_format}")

        absolute_path = os.path.join(settings.MEDIA_ROOT, output_filepath)
